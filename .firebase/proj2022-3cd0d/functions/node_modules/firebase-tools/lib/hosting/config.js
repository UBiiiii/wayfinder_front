...
function validateOne(config, options) {
    ...
    if (root && !(0, fsutils_1.dirExistsSync)((0, projectPath_1.resolveProjectPath)(options, root))) {
        throw new error_1.FirebaseError(`Specified "${config.source ? "source" : "public"}" directory "${root}" does not exist, can't deploy hosting to site "${config.site || config.target || ""}"`);
    }
    ...
    if (config.i18n) {
        ...
        const i18nPath = path_1.default.join(root, config.i18n.root);
