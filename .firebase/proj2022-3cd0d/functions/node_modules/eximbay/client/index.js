...
class EximbayClient {
  ...
  generateFgKey(data = {}) {
    return pipe(commonModule.makeQueryString, query => `${this.secret}?${query}`, d => sha256.create().update(d).hex())(data);
  }

  renderPayment(data = {}) {
    return pipe(data => Object.assign({}, data, {
      ver: 230,
      txntype: 'PAYMENT',
      charset: 'UTF-8',
      mid: this.mid
    }), commonModule.sortByKeys, data => {
      data.fgkey = this.generateFgKey(data);
      return data;
    }, paymentTmpl(`${this.eximbayServerUrl}/Gateway/BasicProcessor.krp`), $.el, $.append($('.request')))(data);
  }
}
