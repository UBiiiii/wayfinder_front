...
exports.PAYMENT_STATUS = exports.RESPONSE_CODE = undefined;
...
var _partialJs2 = _interopRequireDefault(_partialJs);
...
class EximbayServer {
  constructor(secret) {
    this._generateFGKey = _partialJs2.default.pipe(_common.sortByKeys, _common.makeQueryString, query => `${this.secret}?${query}`, d => _crypto2.default.createHash('sha256').update(d).digest('hex'));
    this.requestPaymentResult = _partialJs2.default.pipe(_partialJs2.default.pick(['ver', 'mid', 'ref', 'cur', 'amt', 'transid']), _partialJs2.default.extend({ txntype: 'QUERY', keyfield: 'TRANSID', lang: 'KR', charset: 'UTF-8' }), data => {
      data.fgkey = this._generateFGKey(data);
      return data;
    }, form => new Promise((resolve, reject) => _request2.default.post({ url: 'https://secureapi.test.eximbay.com/Gateway/DirectProcessor.krp', form }, (err, _, body) => err ? reject(err) : resolve(body))), _querystring2.default.parse, _partialJs2.default.mapObject(val => _partialJs2.default.isString(val) ? val.trim() : val));
...
exports.default = EximbayServer;
const RESPONSE_CODE = exports.RESPONSE_CODE = {
  SUCCESS: '0000'
  // 응답 파라미터의 rescode 가 0000이면 정상을 의미하며, 이외의 코드는 거절을 의미합니다. 거절코드 에 대한 상세 내용은 resmsg 를 참고해 주시기 바랍니다.
};

const PAYMENT_STATUS = exports.PAYMENT_STATUS = {
